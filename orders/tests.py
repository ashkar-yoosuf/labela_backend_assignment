...
class OrderViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        Order(user_id=self.user.id, order_id=2, products="1,2,6", date_time="2022-09-30T16:56:00Z").save()
    ...
    def test_order_items_authenticated(self):
        response = self.client.put(reverse("order-handler", kwargs={"pk": 1}), json.dumps({"date_time": "2022-09-28 11:26:00"}), content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content), {'date_time': '2022-09-28T11:26:00Z','order_id': 1,'products': '1,2','user_id': self.user.id})
    ...
    def test_order_view_authenticated(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content), {'delivery_date_time': '2022-09-30T16:56:00Z','items': ['test_product_name', 'test_product_2_name']})
